...
//API setup
const API_KEY = "AIzaSyC9VT8_uq15UtsX97SwJlkC84TYrPfHeZs";
...
//create message element withdynamic classesand return it
...
const generateBotResponce = async (incomingMessageDiv) => {
    ...
    try {
        ...
        const formattedText = apiResponceText.replace(/\n/g, "<br>");

        // Use innerHTML for formatting (preserves <br>)
...
const handleOutgoingMessage = (e) => {
    ...
    // create and display user meassage
...
const pickerOptions = { 
    onEmojiSelect: (emoji) => {
        ...
        messageInput.value = before + emoji.native + after;
        messageInput.focus();
        
        // Optional: Move cursor after the inserted emoji
        messageInput.selectionStart = messageInput.selectionEnd = start + emoji.native.length;
    } 
};
...
// Toggle visibility when clicking the emoji button
...
// Select the Study link in the navbar
const studyLink = document.querySelector(".M"); // Study is 4th link
...
const ChatHome = document.querySelector(".CHATHOME"); // Study is 4th link
// OR use an id for more reliability: <a id="study-link">Study</a>

// Add click event to open the Study page
ChatHome.addEventListener('click', (e) => {
    e.preventDefault(); // prevent default link action if needed
    window.location.href = 'dashboad.html'; // path to your Study page
});
